# GitMetro 전체 뼈대 구성 및 Claude Design Prompt v2

작성일: 2026-05-01
작성 목적: GitHub 저장소 링크를 입력하면 브랜치와 커밋 흐름을 지하철 노선도 또는 게임 스킬트리처럼 한눈에 보여주는 GitMetro의 제품 구조와 Claude Design용 프롬프트를 정리한다.
...
- 최근 입력 저장소 또는 샘플 저장소 영역은 첫 화면이 복잡해지지 않는 선에서 선택 적용한다.
...
- 상단 툴바
  - 저장소명
  - 맵 방향 전환: Horizontal 기본, Vertical 선택
  - 현재 테마 선택
  - Export 버튼
  - GitHub에서 열기 버튼
  - Dark/Light 모드 토글
...
- 중앙 시각화 캔버스
  - pan & zoom 지원
  - 기본값은 가로형 metro map
  - 사용자가 vertical 옵션을 선택하면 세로형 map으로 전환
  - 가로형에서는 메인 라인이 굵고 안정적인 수평선
  - 세로형에서는 메인 라인이 위에서 아래로 이어지는 굵은 중심선
...
주의:
- 이 문서에서 말하는 화면 구분은 프로토타입과 추후 개발 단계를 나누자는 뜻이 아니다.
- 현재 구상한 기능은 가능한 한 한 번의 제품 구현 범위로 보고 설계한다.
- 나중에 새 아이디어가 생기면 기존 md를 수정하지 않고 새 버전 md로 업데이트한다.
...
- `visualMode`: `metro`, `skill-tree`
- `theme`: `gitmetro-dark`, `light`, `cyberpunk`, `london-tube`
- `commitLimit`
- `dateRange`
- `visibleBranchCategories`
...
원칙:
- 기본 orientation은 `horizontal`이다.
- horizontal 모드에서는 `main`을 중앙 수평 노선으로 고정한다.
- horizontal 모드에서는 시간 또는 topological order 기준으로 왼쪽에서 오른쪽으로 흐르게 한다.
- horizontal 모드에서는 보조 브랜치가 main에서 위아래로 분기한다.
- vertical 모드에서는 `main`을 중앙 세로 노선으로 고정한다.
- vertical 모드에서는 시간 또는 topological order 기준으로 위에서 아래로 흐르게 한다.
- vertical 모드에서는 보조 브랜치가 main에서 좌우로 분기한다.
...
8. pan & zoom
9. map orientation 전환: 기본 horizontal, 선택 vertical
10. branch 타입 토글
11. 기본 dark mode
12. 로딩 터미널 애니메이션
13. 에러 상태 화면
...
정리:
- 위 목록은 `프로토타입 이후 언젠가`라는 의미가 아니다.
- 현재 구상에 들어온 기능은 전체 제품 범위로 보고 설계한다.
- 단, 개발 순서는 안정적인 구현을 위해 핵심 필수 기능부터 잡고 이어서 완성 기능을 붙이는 방식이 좋다.
...
- orientation toggle로 horizontal/vertical map 전환
...
- Full app layout with:
  - Top toolbar showing repo name, map orientation toggle, theme selector, export button, GitHub link, dark/light toggle.
...
- The central map should show:
  - default orientation as a horizontal metro map.
  - an available vertical orientation option.
  - in horizontal mode, main as a strong horizontal central line.
  - in vertical mode, main as a strong vertical central line flowing from top to bottom.
...
- Include visual affordance for available theme options:
...
- The design must include a clear map orientation control: Horizontal as default, Vertical as optional.
...
- Include the map orientation control in the component inventory.
...
아래는 디자인 확정 뒤 실제 구현 프롬프트로 확장할 때 사용할 수 있는 방향이다. 아직 코딩 지시가 아니라 구조 설계이다.
구현 순서는 안정적인 개발을 위한 순서일 뿐이며, 최종 제품 범위는 현재 구상 전체를 기준으로 한다.
...
7. Add horizontal/vertical orientation switching.
8. Add pan and zoom.
9. Add tooltip and inspector interactions.
10. Add GitHub API integration after visualization works with mock data.
11. Add OAuth after public repository flow is stable.

Validation:
- Test URL parsing with valid and invalid GitHub inputs.
- Test that main line remains visually dominant.
- Test horizontal map as the default orientation.
- Test vertical map orientation and make sure branch lanes do not overlap.
...
1. 첫 구현 범위에서 `Metro Map`만 먼저 잡을지, `Skill Tree` 시각 모드까지 mock 단계에서 같이 잡을지
2. 첫 구현 순서를 실제 GitHub API 연동부터 할지, mock graph 기반 UI 검증부터 할지
3. OAuth를 초반 구현에 바로 넣을지, 공개 저장소 시각화 안정화 후 붙일지
4. 렌더링 엔진을 SVG로 시작할지, 대형 저장소를 고려해 Canvas를 바로 검토할지
5. 첫 디자인 톤을 `GitMetro Dark`로 고정할지, Cyberpunk 포인트를 더 강하게 줄지
6. vertical map에서 브랜치가 좌우로 갈라지는 형태를 기본으로 할지, 오른쪽 방향으로만 확장할지

권장 기본 선택:
- 첫 구현은 `Metro Map` 중심
- mock graph 기반 UI 검증을 먼저 진행
- 공개 저장소 시각화 안정화 후 OAuth 추가
- SVG + D3 zoom으로 시작
- 기본 디자인은 절제된 dark developer tool 톤
- map orientation은 horizontal을 기본값으로 하고, vertical을 명시적인 전환 옵션으로 제공
